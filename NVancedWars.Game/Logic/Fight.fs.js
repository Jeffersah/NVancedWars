...
export function Fight_damageDealt(attackerHpMult, defenderTerrain, weapon, target) {
    const modifiers = toList(delay(() => append(singleton(new Modifier(0, [attackerHpMult])), delay(() => append(singleton(new Modifier(0, [DamageTypeModule_getDamageCoeff(target.info.armorType, weapon.damageType)])), delay(() => append(singleton(new Modifier(0, [TerrainTypeModule_getDefenseModifier(defaultArg(map((_arg) => (new TerrainType(1, [])), defenderTerrain.building), defenderTerrain.terrainType))])), delay(() => {
        ...
        if (matchValue != null) {
            return singleton(new Modifier(1, [op_UnaryNegation_Int32(matchValue)]));
        }
        else {
            return empty();
        }
...
export function Fight_findBestWeapon(attackerEffectiveHp, attacker, defender, rules, defenderTerrain, range, blind, attributePredicate) {
    ...
    if (!isEmpty(weaponDamages)) {
        ...
        return [(option_2 = tryFind_1((x_1) => (x_1.ammo == null), maxWeapons), (value_2 = head(sortByDescending_1((x_2) => defaultArg(tryFind(x_2.id, attacker.ammo), 0), maxWeapons, {
            Compare: comparePrimitives,
        })), defaultArg(option_2, value_2))), damage];
    }
    else {
        return void 0;
    }
...
export function Fight_previewFight(fight, rules) {
    ...
    patternInput = ((matchValue != null) ? [matchValue[0], matchValue[1]] : [void 0, 0]);
    const atkWeapon = patternInput[0];
    const atkDamage = patternInput[1] | 0;
    let patternInput_1;
    const matchValue_1 = Fight_findBestWeapon(fight.defender.hp - Percentage_op_Multiply_631F3DCD(rules.firstStrikeDamage, atkDamage), fight.defender, fight.attacker, rules, fight.attackerTerrain, fight.range, false, (atkWeapon != null) ? ((atk = atkWeapon, (testWeapon) => Fight_canCounter(atk, testWeapon))) : ((_arg) => true));
    patternInput_1 = ((matchValue_1 != null) ? [matchValue_1[0], matchValue_1[1]] : [void 0, 0]);
    const defWeapon = patternInput_1[0];
    const attackerHpLeft = (fight.attacker.hp - patternInput_1[1]) | 0;
    ...
    const handleState = (unit, hpLeft, spendAmmoFor) => {
        if (hpLeft <= 0) {
            ...
            return new UnitState(unit.owner, unit.id, unit.info, hpLeft, change(spendAmmoFor.id, (_arg_1) => ((_arg_1 != null) ? (_arg_1 - 1) : (void 0)), unit.ammo), unit.position);
        }
        else {
